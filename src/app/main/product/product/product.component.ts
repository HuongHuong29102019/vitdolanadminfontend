...
export class ProductComponent extends BaseComponent implements OnInit {
  ...
  search() { 
    this.page = 1;
    this.pageSize = 5;
    this._api.post('/api/itemAdmin/searchadmin',{page: this.page, pageSize: this.pageSize, item_name: this.formsearch.get('item_name').value, item_color: this.formsearch.get('item_color').value, item_material: this.formsearch.get('item_material').value}).takeUntil(this.unsubscribe).subscribe(res => {
      this.products = res.data;
      this.totalRecords =  res.totalItems;
      this.pageSize = res.pageSize;
      });
